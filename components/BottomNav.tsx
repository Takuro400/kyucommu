...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto z-50">
      <div className="bg-white/85 backdrop-blur-xl border-t border-kpink/10 flex px-1 pt-1 pb-4">
        {NAV.map(({ href, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center justify-center w-10 h-6 rounded-full transition-all duration-200 ${active ? "bg-kpink/15" : ""}`}>
                <Icon
                  size={19}
              ...
              <span className={`text-[9px] font-medium transition-colors duration-200 ${active ? "text-kpink font-semibold" : "text-muted"}`}>
                {label}
              </span>
            </Link>
          );
